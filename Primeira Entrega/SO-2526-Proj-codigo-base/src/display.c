...
/**
 * @brief Inicializa o ambiente ncurses (cores, teclado sem buffer, cursor invisível).
 */
int terminal_init() {
...
/**
 * @brief Desenha o tabuleiro completo, entidades e UI no ecrã.
 * @param mode Define o estado do jogo (Menu, Vitória, Game Over).
 */
...
/**
 * @brief Desenha um caracter numa posição específica (helper function).
 */
...
/**
 * @brief Atualiza o ecrã físico com as alterações feitas no buffer virtual.
 */
void refresh_screen() {
...
/**
 * @brief Lê input do teclado de forma não bloqueante.
 * @return Caracter lido (W, A, S, D, Q, G) ou '\0' se nada for pressionado.
 */
char get_input() {
